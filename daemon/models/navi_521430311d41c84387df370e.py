...
class SpiderBase:
    ...
    def process_response_page(self, request, headers, body):
        ...
        total = int(total)
        page_size = 95
        page_num = total / page_size + (1 if total % page_size > 0 else  0)
        
        params = parse_qs(ups.query)
        
        if not params.has_key("s"):
            for p in range(1, page_num):
                page_url = "%s&s=%s" % (request["url"], p*page_size)
                rets['urls'].append(page_url)
        
        for item in jobj['itemList']:
            rets['items'].append(item['itemId'])
    ...
    def process_rets_pipeline(self, self_obj, rets):
        ...
        for url in rets["urls"]:
            yield self_obj.redis.rpush("list_cat_rets", url)
            
        for url in rets["items"]:
            yield self_obj.redis.lpush("list_items", url)
